...
interface Handler {
    var next1: Handler? //cereri
    var next2: Handler? //raspunsuri
...
class CEOHandler : Handler {
    ...
    override suspend fun handleRequest(mesaj: String) {
        if (mesaj.startsWith("Raspuns")) {
            println("CEO primeste: $mesaj")
            next2?.let {
        ...
        if (priority == 1) {
            println("Sunt CEO si prelucrez: $message")
            delay(300)
            next2?.let {
                CoroutineScope(Dispatchers.Default).launch {
...
class ExecutiveHandler : Handler {
    ...
    override suspend fun handleRequest(mesaj: String) {
        if (mesaj.startsWith("Raspuns")) {
            println("Executive primeste: $mesaj")
            next2?.let {
                CoroutineScope(Dispatchers.Default).launch {
                    it.handleRequest(mesaj)
                }
        ...
        if (priority == 2) {
            println("Sunt Executive si prelucrez: $message")
            delay(300)
            next2?.let {
                CoroutineScope(Dispatchers.Default).launch {
...
class ManagerHandler : Handler {
    ...
    override suspend fun handleRequest(mesaj: String) {
        if (mesaj.startsWith("Raspuns")) {
            println("Manager primeste: $mesaj")
            next2?.let {
                CoroutineScope(Dispatchers.Default).launch {
                    it.handleRequest(mesaj)
                }
        ...
        if (priority == 3) {
            println("Sunt Manager si prelucrez: $message")
            delay(300)
            next2?.let {
                CoroutineScope(Dispatchers.Default).launch {
...
class HappyWorkerHandler : Handler {
    ...
    override suspend fun handleRequest(mesaj: String) {
        if (mesaj.startsWith("Raspuns")) {
            println("HappyWorker primeste: $mesaj")
            next2?.let {
                CoroutineScope(Dispatchers.Default).launch {
                    it.handleRequest(mesaj)
                }
        ...
        if (priority >= 4) {
            println("Sunt HappyWorker si prelucrez: $message")
            delay(300)
            next2?.let {
                CoroutineScope(Dispatchers.Default).launch {
...
class HappyWorkerFactory : AbstractFactory {
    override fun getHandler(handler: String): Handler {
        return when (handler) {
            ...
            else -> throw IllegalArgumentException("Nu exista handler-ul cerut in HappyWorkerFactory")
        }
    }
}
...
fun main() = runBlocking {
    ...
    ceo.handleRequest("4 - Trimite raportul final")
    ceo.handleRequest("2 - Aproba bugetul")
    ceo.handleRequest("1 - Concediaza oameni")
    ceo.handleRequest("3 - Angajeaza stagiar")
